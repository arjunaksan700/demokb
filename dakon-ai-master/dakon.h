...
#define DAKON_PLAYER_PLYR1 0
#define DAKON_PLAYER_PLYR2 1
#define DAKON_PLAYER_PLYR1_NAME "PLYR1"
#define DAKON_PLAYER_PLYR2_NAME "PLYR2"
#define DAKON_INF_SCORE 10000

#define dakon_get_player_name(p) (p == DAKON_PLAYER_PLYR1 ? "PLYR1" : "PLYR2")
...
void dakon_print(board b) {
	...
	printf("          |");
	for(int i = 0; i < DAKON_LUBANG; i++) {
		if(b.lubang[DAKON_PLAYER_PLYR2][i] > 0) {
	...
	printf("PLYR1  %2d |", b.lumbung[DAKON_PLAYER_PLYR1]);
	for(int i = 0; i < DAKON_LUBANG; i++) {
		if(i < DAKON_LUBANG -1) {
			printf("     ");
		} else {
	...
	printf("| %2d  PLYR2\n", b.lumbung[DAKON_PLAYER_PLYR2]);

	printf("          |");
	for(int i = DAKON_LUBANG-1; i >= 0 ; i--) {
		if(b.lubang[DAKON_PLAYER_PLYR1][i] > 0) {
			printf(" %2d |", b.lubang[DAKON_PLAYER_PLYR1][i]);
		} else {
			printf(" -- |");
		}
	}
...
bool dakon_is_playable(board b) {
	...
	while(lubang < DAKON_LUBANG) {
		if(b.lubang[DAKON_PLAYER_PLYR1][lubang] > 0) {
			playable[DAKON_PLAYER_PLYR1] = true;
		}

		if(b.lubang[DAKON_PLAYER_PLYR2][lubang] > 0) {
			playable[DAKON_PLAYER_PLYR2] = true;
		}
...
void dakon_play(board* b, int lubang, int lubang_siapa) {
	...
	assert(lubang_siapa == DAKON_PLAYER_PLYR1 || lubang_siapa == DAKON_PLAYER_PLYR2);
...
int dakon_diff(board b, int player) {
	...
	diff = biji[DAKON_PLAYER_PLYR2] - biji[DAKON_PLAYER_PLYR1];
	diff *= (player == DAKON_PLAYER_PLYR2) ? 1 : -1;
...
int dakon_eval(board b, int player) {
	...
	score = b.lumbung[DAKON_PLAYER_PLYR2] - b.lumbung[DAKON_PLAYER_PLYR1];
	score *= (player == DAKON_PLAYER_PLYR2) ? 1 : -1;
